...
export const AuthProvider = ({ children }) => {
    const getInitialState = () => {
        try {
            const storedAuthData = localStorage.getItem('medisync_auth');
            if (storedAuthData) {
    ...
    const updateUser = (partialUser) => {
        try {
            const current = JSON.parse(localStorage.getItem('medisync_auth')) || { user: null, isAuthenticated: false, token: null };
